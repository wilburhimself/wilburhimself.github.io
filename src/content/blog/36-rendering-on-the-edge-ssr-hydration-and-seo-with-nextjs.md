...
tags: ["nextjs", "ssr", "seo", "performance", "frontend"]
---
...
Server rendering solves this by building the full HTML page on the server _before_ sending it to the browser. The user gets meaningful content immediately, and so do search engines.
...
- **How it works:** The page is served statically. If a request comes in after the `revalidate` period (e.g., 60 seconds), the user gets the stale page, but Next.js triggers a re-generation in the background. The _next_ user gets the fresh page.
...
The trade-off is this: the more data and components your page has, the larger the JavaScript bundle and the longer hydration takes. This can lead to a page that _looks_ ready but isn't actually interactive (a high Time to Interactive - TTI). A user might click a button, and nothing happens.
